...
template<class _View> class SdfChildrenProxy {
 ...
 private:
  ...
  // Sets the value at the given iterator position.
  bool _Set(_inner_iterator pos, const mapped_type &newValue) {
    if (!_Validate(CanSet)) {
      return false;
    }
    SdfChangeBlock block;
    const key_type keyToErase = _view.key(pos);
    const size_t indexToInsert = std::distance(_view.begin(), pos);

    // Erase the old element
    if (!_view.GetChildren().Erase(keyToErase, _type)) {
      return false;
    }

    // Insert the new element at the same index
    if (!_view.GetChildren().Insert(Adapter::Convert(newValue), indexToInsert, _type)) {
      return false;
    }
    return true;
  }
