    ...
    const SdrTokenMap& metadata) const
{
    ...
    std::unordered_map<TfToken, TfToken, TfHash>::const_iterator iter;
    for (iter = contextMapping.begin(); iter != contextMapping.end(); ++iter) {
        if (TfStringContains(TfStringToLower(schemaBase), iter->first)) {
            return iter->second;
        }
    }
