...
public final class CameraController
{
  ...
  public var distance: Double
  {
    ...
    set { camera.params.distance = Swift.max(minimumDistance, Swift.min(maximumDistance, newValue)) }
  }
  ...
  public func frameBoundingBox(_ bbox: Pixar.GfBBox3d)
  {
    ...
    let diagonal = sqrt(bboxSize[0] * bboxSize[0] +
                       bboxSize[1] * bboxSize[1] +
                       bboxSize[2] * bboxSize[2])
  ...
  public func reset()
  {
    ...
    if camera.isZUp
    {
      rotation = Pixar.GfVec3d(-30.0, 45.0, 0.0)
    }
    else
    {
      rotation = Pixar.GfVec3d(-30.0, 45.0, 0.0)
